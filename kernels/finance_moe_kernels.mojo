        ...
        for batch_idx in range(batch_size):
            for seq_idx in range(seq_len):
                ...
                domain_assignments.store[1](
                    domain_idx, SIMD[domain_assignments.dtype, 1](0)
                )

                # Routing probabilities
                for d in range(num_domains):
                    var prob_idx = IndexList[3](batch_idx, seq_idx, d)
                    var prob_val = SIMD[routing_probs.dtype, 1](
                        0.5 if d == 0 else 0.1
                    )
...
struct FinanceMoEFFNOp:
    ...
    fn execute[
        ...
        # Initialize expert utilization output
        var num_experts = expert_utilization.shape()[0]
        for i in range(num_experts):
            var idx = IndexList[1](i)
            expert_utilization.store[1](
                idx, SIMD[expert_utilization.dtype, 1](0.25)
            )
...
struct MarketRegimeDetectorOp:
    ...
    fn execute[
        ...
        price_sequences: InputTensor[dtype = DType.float32, rank=3],
        ...
        for batch_idx in range(batch_size):
            ...
            regime_indicators.store[1](
                ...
                SIMD[regime_indicators.dtype, 1](0.4),
            )
            regime_indicators.store[1](
                IndexList[2](batch_idx, 1),
                SIMD[regime_indicators.dtype, 1](0.3),
            )
            regime_indicators.store[1](
            ...
            volatility_indicators.store[1](
                IndexList[2](batch_idx, 0),
                SIMD[volatility_indicators.dtype, 1](0.5),
            )